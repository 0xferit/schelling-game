...
describe('GameRoom async task tracking', () => {
  ...
  it('preserves start-now votes for forming-match socket replacement', () => {
    const { room } = createRoom();
    const broadcastQueueState = vi
      .spyOn(room, '_broadcastQueueState')
      .mockImplementation(() => {});
    ...
    expect(room.connections.get('acct-1')?.ws).toBe(newWs);
    expect(room.connections.get('acct-1')?.displayName).toBe('Alice Reloaded');
    expect(room.connections.get('acct-1')?.startNow).toBe(true);
    expect(room.formingMatch?.players).toEqual(['acct-1', 'acct-2', 'acct-3']);
    expect(room._buildQueueStateMsg('acct-1')).toMatchObject({
      status: 'forming',
      startNow: true,
      formingMatch: {
        readyHumanCount: 1,
    ...
    room._clearConnectionLivenessMonitor('acct-1');
    if (room.formingMatch?.timer) {
      clearTimeout(room.formingMatch.timer);
      room.formingMatch.timer = null;
    }
  });
