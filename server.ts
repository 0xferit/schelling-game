...
app.post('/api/auth/verify', (req: Request, res: Response) => {
  try {
    ...
    const { sessionToken, account } = verifyChallenge({
      challengeId,
      walletAddress,
      signature,
    });
  ...
  (req: Request, res: Response) => {
    try {
      ...
      if (!displayName || !/^[A-Za-z0-9_-]{1,20}$/.test(displayName)) {
        res
          .status(400)
          .json({ error: 'displayName must match ^[A-Za-z0-9_-]{1,20}$' });
        return;
      }

      const accountState = getAccountState(req.accountId!);
      if (
        accountState.queueStatus === 'queued' ||
        accountState.queueStatus === 'in_match'
      ) {
        res.status(409).json({
          error: 'Cannot change display name while queued or in a match',
        });
        return;
      }
      ...
      const csv = [
        ...
        ...rows.map((r) =>
          headers
            .map((h) =>
              JSON.stringify(
                (r as unknown as Record<string, unknown>)[reverseMap[h]!] ?? '',
              ),
            )
            .join(','),
        ),
...
app.post(
  '/api/admin/leaderboard-eligible',
  requireAdmin,
  (req: Request, res: Response) => {
    try {
