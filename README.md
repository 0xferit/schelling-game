...
Wrangler-managed bindings and most variables live in [wrangler.toml](wrangler.toml). Some optional variables (timeouts) default in code and can be overridden via env vars. The repo defines default and `next` environments. PR previews use a temporary Wrangler config generated by CI.
